...
void render(Aquila::SpectrumType& halfSpectrum,
		cv::Mat& target) {
	cv::rectangle(target, cv::Point(0, 0), cv::Point(WIDTH,HEIGHT), {0,0,0}, CV_FILLED);

	double imagMax = std::numeric_limits<double>().min();
	for(size_t i = 0; i < halfSpectrum.size(); ++i) {
		imagMax = std::max(std::fabs(halfSpectrum[i].imag()), imagMax);
	}
	for(size_t i = 0; i < halfSpectrum.size(); ++i) {
		halfSpectrum[i].real(halfSpectrum[i].real() / 1024 * HEIGHT);
	}

	double realMax = std::numeric_limits<double>().min();
	for(size_t i = 0; i < halfSpectrum.size(); ++i) {
		realMax = std::max(std::fabs(halfSpectrum[i].real()), realMax);
	}

	for(size_t i = 0; i < halfSpectrum.size(); ++i) {
		HSLColor hsl;
		hsl.h_ = 20.0 + (std::fabs(halfSpectrum[i].real() / realMax) * 40.0);
		hsl.s_ = 99;
		hsl.l_ = 50;
		RGBColor rgb(hsl);
		double w = WIDTH/((double)FFT_SIZE / 4);
		if(halfSpectrum[i].real() > 1) {
			//cv::rectangle(target, cv::Point((i - 1) * w,0), cv::Point((i - 1) * w + w - 1,halfSpectrum[i].real() + 1), cv::Scalar(rgb.b_,rgb.g_,rgb.r_), CV_FILLED);
			cv::line(target, cv::Point((i - 1) * w,HEIGHT), cv::Point((i - 1) * w + (((halfSpectrum[i].imag() - imagMax / 2) / (imagMax / 2)) * 5),HEIGHT - (halfSpectrum[i].real() + 1)), cv::Scalar(rgb.b_,rgb.g_,rgb.r_), 2.0 * ceil((double)WIDTH/halfSpectrum.size()));
		}
	}
}
	...
	RecorderCallback rc = [&](AudioWindow& buffer) {
		...
		FramesCollection frames(in, FFT_SIZE);
			for(size_t i = 0; i < frames.count(); ++i) {
				...
				SpectrumType halfSpectrum(halfLength);
				for (std::size_t k = 0; k < halfLength; ++k) {
					halfSpectrum[k] = spectrum[k];
				}

				fbmtx.lock();
				render(halfSpectrum, *frameBuffer);
				fbmtx.unlock();
			}
